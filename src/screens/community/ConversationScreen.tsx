...
export function ConversationScreen() {
  ...
  const { data: messages, isLoading, refetch: refetchMessages } = useMessages(params.conversationId, userId);
  ...
  const onUnsendMessage = (messageId: string, photoPath: string | null) => {
    if (!userId) return;
    Alert.alert("Unsend this message?", "It'll be removed for both of you — this can't be undone.", [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () =>
          deleteMessage.mutate(
            { messageId, conversationId: params.conversationId, userId, photoPath },
            { onError: err => Alert.alert('Could not unsend message', getErrorMessage(err, 'Please try again.')) },
          ),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg.base }}>
            ...
            {(messages ?? []).map(message => {
              ...
              return (
                <View key={message.id} style={{ alignItems: isMine ? 'flex-end' : 'flex-start' }}>
                  {share ? (
                    <Pressable
                      onPress={() => navigation.navigate('SharedWorkoutReview', { shareId: share.id })}
                      style={{ maxWidth: '80%' }}
                    >
                      <View
                        style={{
                      ...
                      >
                        <Icon
                          name="dumbbell"
                          size="sm"
                          color={isMine ? theme.colors.text.onAccent : theme.colors.accent.primary}
                        />
                        <View style={{ flex: 1 }}>
                          <Text
                            variant="body"
                            style={{ fontWeight: '700', color: isMine ? theme.colors.text.onAccent : theme.colors.text.primary }}
                          >
                            {share.title}
                          </Text>
                          <Text
                            variant="caption"
                            style={{ color: isMine ? theme.colors.text.onAccent : theme.colors.text.secondary, opacity: 0.75 }}
                          >
                            {share.share_type === 'weekly_plan' ? '7-day plan' : 'Workout'}
                            {share.status === 'accepted' ? ' · Added to plan' : share.status === 'declined' ? ' · Declined' : ' · Tap to review'}
                          </Text>
                        </View>
                        <Icon
                          name="chevronRight"
                          size="sm"
                          color={isMine ? theme.colors.text.onAccent : theme.colors.text.tertiary}
                        />
                      </View>
                    </Pressable>
                  ) : (
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    {isMine ? (
                      <IconButton
                        ...
                        onPress={() => onUnsendMessage(message.id, message.photo_path)}
                      />
                    ) : null}
                  </View>
                </View>
      ...
      <BottomSheet visible={optionsOpen} onClose={() => setOptionsOpen(false)} title="Conversation Options">
        <View>
          <ListRow title="Delete Conversation" icon="trash" onPress={onDeleteConversation} />
          <ListRow
